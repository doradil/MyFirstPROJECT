trigger ContactTrigger on Contact(
  ...
  for (Contact each : Trigger.new) {
    if (each.Level__c == 'Primary') {
      bookList.add(
        new Book__c(Name = 'Book1' + each.LastName, Contact__c = each.Id)
      );
      bookList.add(
        new Book__c(Name = 'Book2' + each.LastName, Contact__c = each.Id)
      );
      bookList.add(
        new Book__c(Name = 'Book3' + each.LastName, Contact__c = each.Id)
      );
    }
  }
  insert bookList;
